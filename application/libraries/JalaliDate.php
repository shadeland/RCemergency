...
/**
 *
 * CI Jalali Date Library
 * Created by : Adel Shahrezaee
 * Base On  Code By : Mohammad Dayyan
 * Documents : http://www.codeproject.com/Articles/28380/Persian-Calendar-in-PHP
 *
 */
class JalaliDate{
        ...
        while($i<strlen($format))
        {
            ...
            switch ($subtype)
            {
                ...
                case "m":
                    ...
                    break;
                case "M":
                    $result.=self::short_monthname($Dmonth);
                    break;
                case "n":
                    $result1=$Dmonth;
                    if($persianNumber==1) $result.=self::Convertnumber2farsi($result1);
...
//converts the numbers into the persian's number
    ...
    function mcheckdate($month,$day,$year)
    {
        ...
        if($month<=12 && $month>0)
        {
            ...
            if(self::is_kabise($year))
                echo "Asdsd";
            if(self::is_kabise($year) && $m_days_in_month[$month-1]==31)
                return 1;
        }
    ...
    function mgetdate($timestamp="")
    {
        ...
        return array(
            0=>$timestamp,
            "seconds"=>self::mds_date("s",$timestamp),
            "minutes"=>self::mds_date("i",$timestamp),
            "hours"=>self::mds_date("G",$timestamp),
            "mday"=>self::mds_date("j",$timestamp),
            "wday"=>self::mds_date("w",$timestamp),
            "mon"=>self::mds_date("n",$timestamp),
            "year"=>self::mds_date("Y",$timestamp),
            "yday"=>self::days_of_year(self::mds_date("m",$timestamp),self::mds_date("d",$timestamp),self::mds_date("Y",$timestamp)),
            "weekday"=>self::mds_date("l",$timestamp),
            "month"=>self::mds_date("F",$timestamp),
        );
    }
